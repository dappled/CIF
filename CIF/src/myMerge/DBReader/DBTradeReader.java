 ...
 */
public class DBTradeReader implements I_DBReader, Comparable<DBTradeReader> {
	 ...
	 * Constructor
	 ***********************************************************************/
	protected DBTradeReader(final long id, final String filePathName) throws IOException {
		_id = id;
		_isFinished = false;
	...
	private int readOneRecord() throws IOException {
		try {
			...
			_fields = new TradeData( secsFromEpoch, millisecondsFromMidnight, _in.readInt(), _in.readFloat(), _in.readUTF() );
		} catch (final EOFException e) {
			throw new EOFException( "BinaryTradeReader: Input file corrupted: less records than there should be." );
		}
		return 0;
	}
	 ...
	 ***********************************************************************/
	@Override
	public int compareTo(final DBTradeReader otherReader) {
	 ...
	 * Getter and Setter
	 ***********************************************************************/
	public LinkedList<TradeData> getRecordsBuffer() {

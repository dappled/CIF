...
public class PriceRecord {
	...
	public PriceRecord( String dataLine ) throws Exception {
		...
		if( dataFields.length != 7 )
			throw new Exception( String.format( "Less than 7 fields in [%s]", dataLine ) );
		
		// Parse date
		
			String[] dateFields = dataFields[0].split( "/" );
			if( dateFields.length != 3 )
				throw new Exception( String.format( "Bad date field [%s] in record [%s]", dataFields[ 0 ], dataLine ) );
			
			// Convert date to numeric value
			_date = ( Integer.parseInt( dateFields[ 2 ] ) * 10000 ) 
					+ ( Integer.parseInt( dateFields[ 0 ] ) * 100 ) 
		...
		// Parse prices
			...
			_open  = Double.parseDouble( dataFields[ 1 ] );
			_high  = Double.parseDouble( dataFields[ 2 ] );
			_low   = Double.parseDouble( dataFields[ 3 ] );
			_close = Double.parseDouble( dataFields[ 4 ] );
			_adjustedClose = Double.parseDouble( dataFields[ 6 ] );
		...
		// Parse volume
			...
			_volume = Long.parseLong( dataFields[ 5 ] );
